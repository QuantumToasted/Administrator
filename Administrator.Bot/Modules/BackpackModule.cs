...
[SlashGroup("backpack")]
public sealed class BackpackModule(BackpackService backpack, AutoCompleteService autoComplete, SlashCommandMentionService mentions)
    : DiscordApplicationModuleBase
{
            ...
            ItemType type = ItemType.Craftable)
    {
        ...
        if (particleEffect.HasValue)
        {
            if (!dict.TryGetValue(PriceIndex.Unusual(particleEffect.Value), out var unusualPrice))
            {
                ...
                {
                    return await PriceCheckAsync(itemName, quality, particleEffect, ItemType.NonCraftable);
                }
    ...
    public void AutoCompleteItems(AutoComplete<string> itemName, AutoComplete<string> particleEffect)
    {
