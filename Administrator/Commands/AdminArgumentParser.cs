...
{
    public sealed class AdminArgumentParser : IArgumentParser
    {
        public ValueTask<ArgumentParserResult> ParseAsync(CommandContext _)
        {
            ...
            if (arguments != null && arguments.Count != command.Parameters.Count)
            {
                for (var i = arguments.Count; i < command.Parameters.Count; i++)
                {
                    ...
                    if (parameter.Type == typeof(Upload) && context.Message.Attachments.FirstOrDefault() is { } attachment)
                    {
                        arguments[parameter] = (Upload) attachment;
                        continue;
                    }
