...
namespace Administrator.Commands
{
    ...
    public class RoleCommands : AdminModuleBase
    {
        ...
        public AdminCommandResult GetRoleInfo([Remainder] SocketRole role)
        {
            ...
            string FormatMembers()
            {
                ...
                for (var i = 0; i < members.Count; i++)
                {
                    ...
                    if (builder.Length + member.ToString().Length > 1019) // 1024 - 3
                    {
                        builder.Append('…');
                        break;
                    }
        ...
        [Command("id")]
        public AdminCommandResult GetRoleId([Remainder] SocketRole target)
            => CommandSuccessLocalized("rold_id",
                args: new object[] {Format.Bold(target.Name), Format.Code(target.Id.ToString())});
